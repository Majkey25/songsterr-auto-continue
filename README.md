...
The extension watches newly inserted dialogs and text changes. It requires the exact English continuation text, the expected Original Audio heading, and the **Use Synth** + **Upgrade** controls in the same dialog. It waits for the entry animation, rechecks the target, then clicks the continuation once per element. Clicking before the site finishes entering can leave a stuck prompt.
